...
<table border="0" width="100%">
    ...
    <tr>
        ...
        <td width="" height="60" style='padding-left:10px;'>
            ...
            . 출석포인트 : <?php echo number_format($attendance['today_point'])?> 점<br>
			. 개근상 : <?php echo $attendance['day']?>일 : <?php echo number_format($attendance['day_point'])?> 점,&nbsp;&nbsp;	<?php echo $attendance['monthly']?>일 : <?php echo number_format($attendance['monthly_point'])?> 점,&nbsp;&nbsp;<?php echo $attendance['year']?>일 : <?php echo number_format($attendance['year_point'])?> 점<br>
			. 1등 포인트 : <?php echo number_format($attendance['first_point'])?> 점&nbsp;&nbsp;
			. 2등 포인트 : <?php echo number_format($attendance['second_point'])?> 점&nbsp;&nbsp;
			. 3등 포인트 : <?php echo number_format($attendance['third_point'])?> 점
		</td>
    </tr>
</table>
</td>
...
// 메시지 출력
	...
	$mlist = array();
	$mtxt = '';
	@include_once($G5_ATTENDANCE_PATH.'/msg.list.php');
	$mlist_cnt = count($mlist);
	if($mlist_cnt) {
		$mdb = $mlist;
		shuffle($mdb); // 섞기
		@include_once($G5_ATTENDANCE_PATH.'/msg/'.$mdb[0][1]); //첫번째 메시지 파일을 연다.
		$msg_cnt = count($msg);
		if($msg_cnt) {
			shuffle($msg); // 섞기
			$mtxt .= $msg[0];
						...
						<span data-toggle="buttons">
							<?php
								...
								if($mlist_cnt) {
									...
									$mpopover =  "<a href=javascript:apms_msg('life.php')>";
...
<script>
				function apms_msg(q) {
				alert(q)
                						var url = '<?php echo $G5_ATTENDANCE_URL;?>/msg.load.php?q=' + encodeURI(q);
                						$.get(url, function(data) {
                						alert(JSON.stringify(data));
                							$('#subject').val(data.msg);
                						}, "json");
                					}
                									</script>


    <input type="text" id="subject" name="subject" class="input" size="50" value="<?php echo conv_content($mtxt, 0);?>" onmouseover="if(!this.value || this.value == '출석인사를 입력해 주세요.')this.value='';" >
...
<tr height="34">
    ...
    <td align="center" bgcolor="#5bbad6" style='color:#ffffff;font-weight:none;'>출석인사</td>
