			...
			<ul class="btn_bo_user pull-right">
				...
				<li>
					...
					<ul class="more_opt is_list_btn">
						...
						<?php } ?>
						<?php if ($is_checkbox) { ?>
							<li><button type="submit" name="btn_submit" value="수집제외" onclick="document.pressed=this.value"><i class="fa fa-eye-slash" aria-hidden="true"></i> 수집제외</button></li>
...
function fboardlist_submit(f) {
	...
	if(document.pressed == "수집제외") {
		if (!confirm("선택한 게시물을 피드수집에서 제외하시겠습니까?\n\n수집제외된 글은 관리자에게만 노출되며, 마지막 페이지에 출력됩니다."))
			return false;

		f.removeAttribute("target");
		f.action = "<?php echo $board_skin_url;?>/feed.update.php";
	}
